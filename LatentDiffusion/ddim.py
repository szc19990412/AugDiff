...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        D, N = shape
        size = (batch_size, D, N) # B batch_size: 1, D, N
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      unconditional_guidance_scale=1., unconditional_conditioning=None, nu=None):  
        ...
        # https://github.com/mpaepper/stablediffusion_magicmix/blob/main/StableDiffusion_MagicMix.ipynb
        #---->
        ...
        t_min = round(nu * total_steps) # 0.4*20=8
        # t_max = round(0.6 * total_steps)
        layout_steps = total_steps - t_min # 20-8=12
        ...
        for i, step in enumerate(time_range):
            ...
            if (i == layout_steps):
                ts = torch.full((b,), step, device=device, dtype=torch.long)
                img = self.model.q_sample(x_T, ts) #Add noise based on the original input image, introducing noise for ts steps.

            elif (i in fine_tune_steps): 
                pass 
