...
class UnitoData(data.Dataset):
    def __init__(self, dataset_cfg=None,
        ...
        #---->split dataset
        if state == 'train':
            self.data = self.slide_data.loc[:, 'train_image_id'].dropna().tolist()*len(self.type) # larger training dataset, each feature has the choice to be augmented several times
