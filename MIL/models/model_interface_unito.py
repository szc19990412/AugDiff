...
#---->Unitopatho
config = OmegaConf.load("/data111/shaozc/Diffusion/latent-diffusion-main/configs/latent-diffusion/unitopatho.yaml")  
aug_model_pre = load_model_from_config(config, "/data111/shaozc/Diffusion/latent-diffusion-main/logs/2022-12-28T13-58-29_unitopatho/checkpoints/epoch=000111.ckpt") #512
#---->uncondition
...
class ModelInterfaceunito(pl.LightningModule):

    #---->init
    def __init__(self, model, loss, optimizer, **kargs):
        super(ModelInterfaceunito, self).__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            uc = aug_model.get_learned_conditioning(
                {aug_model.cond_stage_key: torch.tensor(n_samples_per_class*[class_label]).to(torch.device("cuda"))}
                )

            # #---->random select a class
            classes = torch.tensor([0, 1, 2, 3, 4, 5, 6])  # define classes to be sampled here
            class_label = np.random.choice(np.arange(len(classes)), size=1)[0]

            # if class_label !=0:

            xc = classes[class_label].repeat_interleave(n_samples_per_class)
            # c = self.model.get_learned_conditioning({self.model.cond_stage_key: xc.to(self.model.device)})
            c = aug_model.get_learned_conditioning({aug_model.cond_stage_key: xc.to(torch.device("cuda"))})        
            ...
            samples_ddim, _ = sampler.sample(S=self.total_steps,
                                            ...
                                            verbose=False,
                                            #---->in the diffusion training process, we need to set "n_classes+1"
                                            unconditional_guidance_scale=self.u_scale, 
                                            nu = self.scale,
                                            unconditional_conditioning=uc, 
                                            x_T=WSI_pt, #original feature
                                            eta=0.0)
    ...
    def configure_optimizers(self):
        ...
        # optimizer = create_optimizer_two(self.optimizer, self.model, self.coattn)
        return [optimizer]
